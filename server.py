...
rus_tail = df_rus.tail(1)
rus_row_count = int(df_rus["С кем Вы путешествуете?"].count())
eng_tail = df_eng.tail(1)
eng_row_count = int(df_eng["С кем Вы путешествуете?"].count())
...
@app.get("/metadata/{language}")
async def metadata(language: str):
  print(rus_row_count)

  if language == "rus":
    d = {"date": rus_tail["Дата"].values[0], "count": rus_row_count}
  elif language == "eng":
    d = {"date": eng_tail["Дата"].values[0], "count": eng_row_count}

  return JSONResponse(content=(d), headers={"Access-Control-Allow-Origin": "*"})